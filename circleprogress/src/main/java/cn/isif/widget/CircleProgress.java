...
public class CircleProgress extends View {
    ...
    protected void initByAttributes(TypedArray attributes) {
        finishedStrokeColor = attributes.getColor(R.styleable.CircleProgress_c_finished_color, default_finished_color);
        unfinishedStrokeColor = attributes.getColor(R.styleable.CircleProgress_c_unfinished_color, default_unfinished_color);
        textColor = attributes.getColor(R.styleable.CircleProgress_c_text_color, default_text_color);
        textSize = attributes.getDimension(R.styleable.CircleProgress_c_text_size, default_text_size);

        setMax(attributes.getInt(R.styleable.CircleProgress_c_max, default_max));
        setProgress(attributes.getInt(R.styleable.CircleProgress_c_progress, 0));
        finishedStrokeWidth = attributes.getDimension(R.styleable.CircleProgress_c_finished_stroke_width, default_stroke_width);
        unfinishedStrokeWidth = attributes.getDimension(R.styleable.CircleProgress_c_unfinished_stroke_width, default_stroke_width);
        if (attributes.getString(R.styleable.CircleProgress_c_prefix_text) != null) {
            prefixText = attributes.getString(R.styleable.CircleProgress_c_prefix_text);
        }
        if (attributes.getString(R.styleable.CircleProgress_c_suffix_text) != null) {
            suffixText = attributes.getString(R.styleable.CircleProgress_c_suffix_text);
        }
        if (attributes.getString(R.styleable.CircleProgress_c_text) != null) {
            text = attributes.getString(R.styleable.CircleProgress_c_text);
        }
        innerBackgroundColor = attributes.getColor(R.styleable.CircleProgress_c_background_color, default_inner_background_color);

        innerBottomTextSize = attributes.getDimension(R.styleable.CircleProgress_c_inner_bottom_text_size, default_inner_bottom_text_size);
        innerBottomTextColor = attributes.getColor(R.styleable.CircleProgress_c_inner_bottom_text_color, default_inner_bottom_text_color);
        innerBottomText = attributes.getString(R.styleable.CircleProgress_c_inner_bottom_text);

        startingDegree = attributes.getInt(R.styleable.CircleProgress_c_circle_starting_degree, default_startingDegree);
    }
